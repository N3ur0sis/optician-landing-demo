...
import { LandingPageSettings, defaultApparenceSettings } from '@/types/apparence';

export default function Page() {
  const { scroll, setScroll } = useContext(ScrollContext);
  const { settings: apparenceSettings, isLoaded: settingsLoaded } = useApparence();
  ...
  const branding: LandingPageSettings = {
    ...
    intro_scroll_indicator_enabled: apparenceSettings.intro_scroll_indicator_enabled,
  ...
  const mobileLogoY = useTransform(scroll, v => `${-800 * Math.pow(v as number, 1.8)}px`); // Logo goes UP
  const mobileTextY = useTransform(scroll, v => `${800 * Math.pow(v as number, 1.6)}px`); // Text goes DOWN
    ...
    <main className="relative min-h-[300vh] w-full overflow-x-hidden cursor-custom" ref={heroRef}>
          ...
          >
            ...
            >
              <motion.div
                ...
                animate={branding.loading_animation_type === 'pulse' ? { 
                  scale: [1, 1.05, 1],
                  opacity: [0.9, 1, 0.9]
      ...
      <div 
        ...
        style={{
          background: branding.intro_gradient_enabled 
            ? `radial-gradient(circle at center, #ffffff 0%, ${branding.intro_gradient_color}${Math.round((branding.intro_gradient_intensity / 100) * 40).toString(16).padStart(2, '0')} 30%, ${branding.intro_gradient_color}${Math.round((branding.intro_gradient_intensity / 100) * 80).toString(16).padStart(2, '0')} 60%, ${branding.intro_gradient_color} 100%)`
        ...
        <motion.div className="flex gap-2 sm:gap-3 mb-2" variants={containerVariants}>
          ...
          {branding.social_instagram && (
            <motion.a
              ...
              className="text-lg sm:text-xl text-black/70 hover:text-black transition-colors duration-300 rounded-full border border-black/10 hover:border-black/40 p-1.5 sm:p-2 cursor-pointer shadow-none"
              variants={socialItemVariants}
              transition={{ duration: 0.5, type: "spring", bounce: 0.6, delay: 0.1 }}
              whileHover={{ scale: 1.1, y: -2 }}
              whileTap={{ scale: 0.95 }}
            >
              <FaInstagram />
      ...
      <section className="relative w-full min-h-screen z-10 overflow-hidden">
        ...
        <motion.div
          ...
          animate={isLoaded ? { opacity: 1, x: 0, y: 0 } : { opacity: 0, x: isMobile ? 0 : -60, y: isMobile ? -30 : 0 }}
        ...
        >
          <motion.div
            ...
            animate={isLoaded ? { y: '0%', scale: 1 } : { y: isMobile ? '-50%' : '100%', scale: 0.8 }}
