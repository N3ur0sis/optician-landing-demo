...
import { SPACING_OPTIONS, getSpacingPx, getSpacingIndexFromValue } from './types';

// Unit type for spacing
type SpacingUnit = 'px' | '%';

// Parse value to get unit and numeric value
function parseSpacingValue(value: string | undefined): { unit: SpacingUnit; numericValue: number } {
  if (!value || value === 'none') return { unit: 'px', numericValue: 0 };
  if (value === 'auto') return { unit: 'px', numericValue: 0 };
  if (value.includes('%')) return { unit: '%', numericValue: parseInt(value) || 0 };
...
function SpacingControl({
  ...
  const c = color === 'orange'
    ? { accent: 'accent-orange-500', bg: 'bg-orange-100', active: 'bg-orange-500', border: 'border-orange-300', text: 'text-orange-700', switchBg: 'bg-orange-500' }
    : { accent: 'accent-green-500', bg: 'bg-green-100', active: 'bg-green-500', border: 'border-green-300', text: 'text-green-700', switchBg: 'bg-green-500' };

  const displayValue = isAuto ? 'auto' : unit === '%' ? `${numericValue}%` : `${numericValue}px`;
    ...
    <div className="space-y-2 p-2 bg-gray-50 rounded-lg border border-gray-200">
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          <span className={`text-xs px-1.5 py-0.5 rounded ${c.bg} ${c.text} font-mono min-w-[45px] text-center`}>
            {displayValue}
          </span>
            ...
            <div className="flex rounded-md overflow-hidden border border-gray-300">
              <button
                ...
                className={`px-2 py-0.5 text-[10px] font-medium transition-colors ${
                  unit === 'px' ? `${c.switchBg} text-white` : 'bg-white text-gray-600 hover:bg-gray-100'
                }`}
              >px</button>
              <button
                onClick={() => handleUnitChange('%')}
                className={`px-2 py-0.5 text-[10px] font-medium transition-colors ${
                  unit === '%' ? `${c.switchBg} text-white` : 'bg-white text-gray-600 hover:bg-gray-100'
                }`}
              >%</button>
            </div>
          )}
          ...
          {showAuto && (
            <button
              ...
              className={`px-2 py-0.5 text-[10px] font-medium rounded border transition-colors ${
                isAuto ? `${c.active} text-white border-transparent` : 'bg-white text-gray-600 border-gray-300 hover:bg-gray-100'
              }`}
            >auto</button>
          )}
        </div>
...
function BoxModelVisual({
  ...
  const getSideClasses = (side: 'top' | 'right' | 'bottom' | 'left', baseClass: string) => 
    `${baseClass} ${c.bg} cursor-pointer transition-all flex items-center justify-center ${
      activeSide === side ? 'ring-2 ring-blue-500 ring-inset' : 'hover:brightness-95'
    }`;
  ...
  return (
    <div className={`relative border-2 ${c.border} rounded-lg overflow-hidden`}>
      ...
      >
        <span className={`text-[10px] font-medium ${c.text}`}>{getDisplay('top')}</span>
      </div>
      <div className="flex items-stretch">
        ...
        >
          <span className={`text-[10px] font-medium ${c.text}`}>{getDisplay('left')}</span>
        </div>
        <div className="flex-1 flex items-center justify-center py-2 bg-white border-x border-gray-200">
          <span className="text-[10px] text-gray-400">{isMargin ? 'M' : 'P'}</span>
        </div>
        <div
          className={getSideClasses('right', 'w-10 py-2')}
          onClick={() => onSelectSide(activeSide === 'right' ? null : 'right')}
        >
          <span className={`text-[10px] font-medium ${c.text}`}>{getDisplay('right')}</span>
        </div>
      </div>
      <div
      ...
      >
        <span className={`text-[10px] font-medium ${c.text}`}>{getDisplay('bottom')}</span>
      </div>
    </div>
  );
}
...
export default function SpacingEditor({ styles, updateStyles }: SpacingEditorProps) {
  const [activeMarginSide, setActiveMarginSide] = useState<'top' | 'right' | 'bottom' | 'left' | null>(null);
  const [activePaddingSide, setActivePaddingSide] = useState<'top' | 'right' | 'bottom' | 'left' | null>(null);
  ...
  const handleMarginChange = useCallback((side: 'top' | 'right' | 'bottom' | 'left', value: string | undefined) => {
    updateStyles(`margin${side.charAt(0).toUpperCase() + side.slice(1)}`, value);
  }, [updateStyles]);

  const handlePaddingChange = useCallback((side: 'top' | 'right' | 'bottom' | 'left', value: string | undefined) => {
    updateStyles(`padding${side.charAt(0).toUpperCase() + side.slice(1)}`, value);
  }, [updateStyles]);
  ...
  const sideLabels: Record<string, string> = { top: 'Haut', right: 'Droite', bottom: 'Bas', left: 'Gauche' };
    ...
    <div className="space-y-4">
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <button onClick={resetMargins} className="text-[10px] text-gray-400 hover:text-red-500 flex items-center gap-0.5" title="Reset">
        ...
        {activeMarginSide && (
          <SpacingControl
            ...
            color="orange"
            showAuto={activeMarginSide === 'left' || activeMarginSide === 'right'}
          />
        )}
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <button onClick={resetPaddings} className="text-[10px] text-gray-400 hover:text-red-500 flex items-center gap-0.5" title="Reset">
            <RotateCcw className="w-3 h-3" />
          </button>
        </div>
