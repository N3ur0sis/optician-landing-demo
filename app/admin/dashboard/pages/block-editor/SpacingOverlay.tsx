...
import { SPACING_OPTIONS, getSpacingPx, getSpacingValueFromIndex, getSpacingIndexFromValue } from './types';
...
function getZoneStyle(
  ...
  const baseStyle: React.CSSProperties = {
    position: 'absolute',
    backgroundColor: isMargin ? 'rgba(251, 146, 60, 0.4)' : 'rgba(74, 222, 128, 0.4)',
  ...
  if (isMargin) {
    switch (side) {
      ...
      case 'bottom':
        return { ...baseStyle, bottom: 0, left: 0, right: 0, height: displaySize };
      case 'left':
        return { ...baseStyle, top: margins.top, bottom: margins.bottom, left: 0, width: displaySize };
      case 'right':
        return { ...baseStyle, top: margins.top, bottom: margins.bottom, right: 0, width: displaySize };
    }
  } else {
    switch (side) {
      case 'top':
        return { ...baseStyle, top: margins.top, left: margins.left, right: margins.right, height: displaySize };
      case 'bottom':
        return { ...baseStyle, bottom: margins.bottom, left: margins.left, right: margins.right, height: displaySize };
      case 'left':
        return { ...baseStyle, top: margins.top + paddings.top, bottom: margins.bottom + paddings.bottom, left: margins.left, width: displaySize };
      case 'right':
        return { ...baseStyle, top: margins.top + paddings.top, bottom: margins.bottom + paddings.bottom, right: margins.right, width: displaySize };
    }
  }
  return null;
}
...
export default function SpacingOverlay({ 
  ...
  const [isDragging, setIsDragging] = useState<{ type: 'margin' | 'padding'; side: 'top' | 'right' | 'bottom' | 'left' } | null>(null);
  ...
  const dragStartRef = useRef<{ x: number; y: number; startValue: number; isPercent: boolean }>({ x: 0, y: 0, startValue: 0, isPercent: false });
  ...
  const toPixels = useCallback((value: string | undefined, dimension: 'width' | 'height' = 'width'): number => {
    if (!value || value === 'none' || value === 'auto') return 0;
    if (value.includes('%')) {
      const percent = parseInt(value) || 0;
      const size = dimension === 'width' ? dimensions.width : dimensions.height;
  ...
  // Drag handling
  const handleDragStart = useCallback((e: React.MouseEvent, type: 'margin' | 'padding', side: 'top' | 'right' | 'bottom' | 'left') => {
    e.preventDefault();
    e.stopPropagation();
    const key = `${type}${side.charAt(0).toUpperCase() + side.slice(1)}` as keyof BlockStyles;
    const value = styles[key] as string;
    const isPercent = value?.includes('%');
    const startValue = isPercent ? (parseInt(value) || 0) : getSpacingIndexFromValue(value);
    dragStartRef.current = { x: e.clientX, y: e.clientY, startValue, isPercent };
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const delta = isH
        ? (isDragging.side === 'left' ? e.clientX - dragStartRef.current.x : dragStartRef.current.x - e.clientX)
        : (isDragging.side === 'top' ? e.clientY - dragStartRef.current.y : dragStartRef.current.y - e.clientY);
      ...
      if (isPercent) {
        ...
        const newPercent = Math.max(0, Math.min(50, dragStartRef.current.startValue + percentDelta));
        // Only update if value actually changed
        if (newPercent !== lastValueRef.current) {
          lastValueRef.current = newPercent;
          setCurrentDragValue(newPercent);
          onUpdateStyleRef.current(key, newPercent === 0 ? undefined : `${newPercent}%`);
        }
      } else {
        const indexDelta = Math.round(delta / 10);
        const newIndex = Math.max(0, Math.min(SPACING_OPTIONS.length - 1, dragStartRef.current.startValue + indexDelta));
        // Only update if value actually changed
  ...
  const isActive = (t: 'margin' | 'padding', s: 'top' | 'right' | 'bottom' | 'left') => 
    isDragging?.type === t && isDragging?.side === s;

  const hasNoSpacing = m.top === 0 && m.right === 0 && m.bottom === 0 && m.left === 0 && 
                       p.top === 0 && p.right === 0 && p.bottom === 0 && p.left === 0;

  // Render a spacing zone
  const renderZone = (type: 'margin' | 'padding', side: 'top' | 'right' | 'bottom' | 'left', size: number, label: string) => {
    ...
    return (
      ...
      >
        {size > 0 && (
          <span className={`px-1 py-0.5 rounded text-[9px] font-medium text-white whitespace-nowrap ${
    ...
    <>
      ...
      {isDragging && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 bg-gray-900 text-white text-sm px-3 py-1.5 rounded-full pointer-events-none flex items-center gap-2">
          <span className={`w-2.5 h-2.5 rounded-full ${isDragging.type === 'margin' ? 'bg-orange-400' : 'bg-green-400'}`} />
          <span className="text-xs">
            {isDragging.type === 'margin' ? 'Marge' : 'Padding'} {isDragging.side === 'top' ? 'haut' : isDragging.side === 'bottom' ? 'bas' : isDragging.side === 'left' ? 'gauche' : 'droite'}
          </span>
          <span className="font-mono text-xs bg-white/10 px-1.5 py-0.5 rounded">
            {isPercentMode ? `${currentDragValue}%` : `${SPACING_OPTIONS[currentDragValue]?.px || 0}px`}
          </span>
        </div>
      )}
