...
async function seedStoresV3() {
  console.log('🏪 Seeding store pages with STORE_LAYOUT block...\n')
  ...
  for (const store of stores) {
    ...
    // Create the page with STORE_HERO + STORE_LAYOUT
    const page = await prisma.page.create({
      data: {
        ...
        blocks: {
          create: [
            // STORE_HERO - Hero section with store name
            {
              type: 'STORE_HERO',
              order: 0,
              visible: true,
            ...
            // STORE_LAYOUT - Main content with 2 columns
            {
              type: 'STORE_LAYOUT',
              order: 1,
              visible: true,
